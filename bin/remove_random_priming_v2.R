...
if(!opt$org %in% c("hg38", "mm10", "mm39", "rn6", "rn7")) {
  print_help(opt_parser)
  stop("--org should be one of [hg38, mm10, mm39, rn6, rn7].")
}

# opt <- list(bg_pos = "pos.bedgraph.gz",
#             bg_neg = "neg.bedgraph.gz",
#             minreads = 2,
#             clusterdist = 200,
#             pas = 0.5,
#             apa = 0.4,
...
# Get peak position and max scores
polya.reduced.gr <- reduce(polya.gr, with.revmap = TRUE)
revmap <- mcols(polya.reduced.gr)$revmap 

polya.dt <- as.data.table(polya.gr)
polya.dt$reduced_id <- rep(paste0("ID", seq_along(polya.reduced.gr)), elementNROWS(polya.reduced.gr$revmap))
polya.dt$peak_start <- rep(start(polya.reduced.gr), elementNROWS(polya.reduced.gr$revmap))
polya.dt$peak_end <- rep(end(polya.reduced.gr), elementNROWS(polya.reduced.gr$revmap))

setkey(polya.dt, reduced_id)
polya.dt[, max_score := max(score), by = reduced_id]
polya.reduced.dt <- polya.dt[score == max_score]

polya.reduced.pos.dt <- polya.reduced.dt[strand == "+"]
setorder(polya.reduced.pos.dt, seqnames, -end) # i.e. 3' to 5'
polya.reduced.pos.unique.dt <- unique(polya.reduced.pos.dt, by = "reduced_id") # will pick first, i.e. 3' most

polya.reduced.neg.dt <- polya.reduced.dt[strand == "-"]
setorder(polya.reduced.neg.dt, seqnames, start) # i.e. 3' to 5'
polya.reduced.neg.unique.dt <- unique(polya.reduced.neg.dt, by = "reduced_id") # will pick first, i.e. 3' most


polya.reduced.unique.dt <- rbindlist(list(polya.reduced.pos.unique.dt, polya.reduced.neg.unique.dt))
polya.reduced.unique.gr <- GRanges(polya.reduced.unique.dt)
polya.gr <- sort(polya.reduced.unique.gr)

# polya.reduced.grl <- relist(polya.gr[unlist(revmap)], revmap)
# 
# polya.reduced.max.grl <- mclapply(seq_along(polya.reduced.grl), function(i) {
#   
#   # message(i)
#   
#   gr <- polya.reduced.grl[[i]]
#   gr <- gr[gr$score == max(gr$score)] # Select coordinate with the most reads
#   
#   # Break ties by selecting 3' most coordinate
#   if(unique(as.character(strand(gr))) == "+") {
#     
#     gr <- gr[length(gr)]
#     
#   } else if(unique(as.character(strand(gr))) == "-") {
#     
#     gr <- gr[1]
#     
#   }
...
polya.dt <- as.data.table(polya.gr)
polya.dt[, A_content := str_count(str_sub(sequence, 61, 80), "A")/20]

# Plot A content distribution
p <- ggplot(polya.dt, aes(x = A_content)) +
  geom_histogram(binwidth = 0.1, fill = "steelblue4", ) +
...
polya.dt[, bin_A := cut(polya.dt$A_content, breaks = thresholds, include.lowest = TRUE, labels = bin.names)]
# polya.dt$bin_A <- cut(polya.dt$A_content, breaks = thresholds, include.lowest = TRUE, labels = bin.names)
stopifnot(all(!is.na(polya.dt$bin_A))) # make sure all allocated
...
polya.dt[, upstream.seq := str_sub(sequence, 20, 60)]
...
# polya.dt[, width := NULL]
# polya.dt[, name := "."]
polya.dt[, name := reduced_id]
setcolorder(polya.dt, c("seqnames", "start", "end", "name", "score", "strand", "peak_start", "peak_end", "max_score", "sequence", "A_content", "bin_A", "PAS"))
...
polya.filtered.bed[, start := start + 1L] # shift back before merging
bg <- GRanges(polya.filtered.bed)

# Merge peak regions rather than peaks
bg$peak <- start(bg)
start(bg) <- bg$peak_start
end(bg) <- bg$peak_end

bed <- reduce(bg, min.gapwidth = as.integer(opt$clusterdist)) # merge ones within cluster distance of each other
bed$id <- paste0("PAS", str_pad(1:length(bed), width = nchar(length(bed)), side = "left", pad ="0"))
...
unique.bg <- rbindlist(list(pos.bg, neg.bg))
stopifnot(all(bg$id %in% unique.bg$id))
stopifnot(all(!duplicated(unique.bg$id)))

unique.bg[, id := paste0(id, "_", PAS, "_", A_content)]
unique.bg <- GRanges(unique.bg)
unique.bg$name <- unique.bg$id

# Create peak region with thick part as peak
unique.bg$thick <- IRanges(start = unique.bg$peak, end = unique.bg$peak)
export.bed(unique.bg, "peak.bed")
